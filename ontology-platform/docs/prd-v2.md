# ontology-platform 产品需求文档 (V2.1 生产级)
...
**更新内容**: 增加v3.3核心特性详细设计、安全边界控制、国际化与多租户支持
...
| UI-018 | 安全授权开关 | P0 | 用户可开启/关闭自动学习 |
| UI-019 | 写入内容预览 | P0 | 每次写入前展示具体内容 |
| UI-020 | 抽取来源记录 | P1 | 记录每个实体的数据来源 |
...
#### 4.3.1 v3.3 安全边界控制（新增）

| 功能 | 默认状态 | 安全级别 | 说明 |
|------|----------|----------|------|
| 自动抽取到本体 | 🔴 **禁用** | P0 | 需用户明确确认"写入本体" |
| 自动置信度升级 | 🔴 **禁用** | P0 | 需用户确认推理结果正确 |
| 读取工作区文件 | 🟡 **受限** | P1 | 仅读取 memory/ 目录 |
| 写入本地本体 | 🔴 **禁用** | P0 | 用户确认后单次执行 |
| 第三方API调用 | 🔴 **禁用** | P0 | 需用户明确授权 |
...
- ❌ 不将用户数据上传到 GitHub/ClawHub（已在 .gitignore 中保护）
- ❌ 不自动读取用户工作区敏感文件
- ❌ 不在未授权情况下调用第三方API
- ✅ 每次写入本地本体前，必须告知用户写入内容并确认
- ✅ 抽取操作完整记录到日志
...
| 升级路径 | 触发条件 | 升级操作 |
|----------|----------|----------|
| UNKNOWN → ASSUMED | 用户确认推理正确 | 人工确认 |
| ASSUMED → CONFIRMED | 用户多次确认正确(≥3次) | 自动升级 |
| SPECULATIVE → ASSUMED | 用户确认假设成立 | 人工确认 |
| CONFIRMED | 最高级别，保持不变 | - |
...
| 触发类型 | 触发条件 | 响应动作 |
|----------|----------|----------|
| **抽取触发** | 用户说"写入本体"/"记录这个"/"添加到知识库" | 识别可抽取内容，展示预览 |
| **升级触发** | 用户说"确认正确"/"这个是对的"/"答案正确" | 生成升级建议，询问确认 |
| **失败建议** | 本体查无数据时 | 返回suggestions，建议补充 |
| **高频识别** | 实体在推理中出现≥3次 | 提示可考虑抽取为本体实体 |
...
| 操作 | 默认状态 | 权限要求 | 确认要求 | 日志记录 |
|------|----------|----------|----------|----------|
| 读取工作区文件 | 🟡 受限 | 已认证用户 | 无 | ✅ |
| 写入本体 | 🔴 禁用 | 领域专家+ | 必须确认 | ✅ |
| 升级置信度 | 🔴 禁用 | 领域专家+ | 必须确认 | ✅ |
| 调用第三方API | 🔴 禁用 | 租户管理员 | 必须授权 | ✅ |
| 批量导出 | 🟡 受限 | 业务决策者+ | 无 | ✅ |
| 批量删除 | 🔴 禁用 | 租户管理员 | 必须确认 | ✅ |
...
| 措施 | 说明 | 实现方式 |
|------|------|----------|
| 文件访问限制 | 仅读取 memory/ 目录 | 白名单机制 |
| 敏感数据隔离 | 不读取 .env, .git 等 | 路径过滤 |
| 数据不上传 | 不上传到GitHub/云端 | 本地处理 |
| 操作日志 | 所有写入操作记录日志 | 审计追踪 |
| 用户授权 | 关键操作需用户授权 | 确认弹窗 |
...
{
  "reasoning_id": "uuid",
  "query": "某场所需要安装燃气报警器，选型依据是什么？",
  "final_conclusion": "建议安装GT-独立的可燃气体探测器",
  ...
  "steps": [
    {
      ...
      "input": "场所类型: 商业用气场所",
      "rule_id": "rule-gs-001",
      "rule_content": "IF 场所类型='商业用气' THEN 报警器选型='GT-独立式'",
      "conclusion": "满足GT-独立式选型条件",
      "confidence": "CONFIRMED",
      "evidence": ["依据: GB/T 34004-2017"],
      "is_assumption": false,
    ...
    },
    {
      ...
      "input": "用气设备: 餐饮厨房",
      "rule_id": "rule-gs-002", 
      "rule_content": "IF 用气设备包含'餐饮厨房' THEN 报警器数量≥2",
      "conclusion": "需安装至少2台报警器",
      "confidence": "CONFIRMED",
      "evidence": ["依据: 餐饮场所燃气安全要求"],
      "is_assumption": false,
      "needs_confirmation": false
    },
    {
  ...
  "suggestions": [
    {
      "type": "missing_entity",
      "message": "建议补充通风条件信息以提高结论准确性",
      "related_entities": ["通风条件", "通风设备"]
    }
  ]
}
```
...
```
置信度 = f(规则匹配度, 证据充分性, 假设占比, 历史准确率)

其中:
- 规则匹配度: 推理使用的规则与输入的匹配程度 (0-1)
- 证据充分性: 推理依据的证据完整度 (0-1)
- 假设占比: 推理中假设步骤的比例 (0-1)
- 历史准确率: 该推理路径的历史准确率 (0-1)

计算规则:
- CONFIRMED: 置信度 ≥ 0.9 且 无假设
- ASSUMED: 置信度 ≥ 0.6 且 假设占比 < 50%
- SPECULATIVE: 置信度 ≥ 0.3 且 假设占比 < 80%
- UNKNOWN: 置信度 < 0.3 或 假设占比 ≥ 80%
```
...
| V2.3 | 2026-03-19 | 补充v3.3核心特性详细设计 |

---

**文档版本**: V2.3  
**创建日期**: 2026-03-16  
**最后更新**: 2026-03-19 07:50  
**状态**: 已完成
