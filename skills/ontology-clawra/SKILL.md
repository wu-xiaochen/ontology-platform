...
description: Palantir本体论实践版本 - **通用领域综合版**。结构化知识图谱+规则引擎+科学推理方法论+自动知识抽取。用于推理计算、选型分析、知识库查询。**推理结果必须展示详细推理过程**：包含用户需求、每一步的规则依据ID、具体的规则内容、计算公式、参数来源、置信度标注。**自动学习**：当用户确认推理结果时，自动抽取新实体到本体。**多领域支持**：供应链、医疗、金融、制造、法律、能源等54+领域本体库。触发关键词：推理、选型、计算、分析、知识库、本体、学习、抽取。
metadata:
  {
    "openclaw": {
      "emoji": "🧠",
...
AUTO_LEARN_TRIGGERS:
  ...
  - event: "user_confirms_reasoning"
    ...
    upgrade_confidence: "ASSUMED → CONFIRMED"
  
  # 触发2：高频实体出现
  - event: "entity_mentioned_3_times"
    action: "create_index"
    threshold: 3
...
1. check_ontology("供应商A 风险评估")
   → 结果：
   - 本体有供应商评估维度数据
   - 有风险指标规则
   → 标注：CONFIRMED

2. 运行 supplier_risk_evaluation.py
   → 风险评分：95/100（高风险）
   → 风险因子：OTD<90%、不良率>2%、单一供应商占比>60%
   → 应对建议：13条
...
1. check_ontology("物料B 价格分析")
   → 本体有历史价格数据
   → 有价格分析规则

2. 运行 purchase_price_analysis.py
   → 价格竞争力：与市场均价持平
   → 趋势：近3个月上涨5%
   → 议价空间：中等（采购量50k）
...
1. check_ontology("合同条款 风险审查")
   → 本体有合同审查规则

2. 运行 contract_review.py
   → 风险点：预付款35%（>30%高风险）
   → 交付周期：60天（>45天高风险）
   → 质保期：6个月（<12个月中风险）
