...
description: Palantir本体论实践版本 v3.3 - **新增主动学习能力**。结构化知识图谱+规则引擎+科学推理方法论+自动知识抽取。用于推理计算、选型分析、知识库查询。**推理结果必须展示详细推理过程**：包含用户需求、每一步的规则依据ID、具体的规则内容（不是名称）、计算公式、参数来源、置信度标注。**自动学习**：当用户确认推理结果时，自动抽取新实体到本体。触发关键词：推理、选型、计算、分析、知识库、本体、燃气调压、燃气报警器、学习、抽取。
metadata:
  {
    "openclaw": {
      "emoji": "🧠",
